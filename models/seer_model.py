...
def generate_attention_mask_my(K, num_A, num_B, atten_goal, atten_goal_state,
    ...
    for i in range(K):
        ...
        # text + state + image -> action 
        attention_mask[start_index+num_A+num_obs_token:end_index, start_index+2:start_index+3] = -float('inf')

        # action + state + image -> image   ##TODO question: here, pred_obs have no effects on pred_action, and vice versa. 
        attention_mask[start_index+num_A:start_index+num_A+num_obs_token, start_index:start_index+1] = -float('inf')
...
class SeerAgent(nn.Module):
    def __init__(
        ...
        # text projector
        self.text_projector = nn.Linear(512, self.hidden_dim) ## 384
        ...
        self.state_projector = nn.Linear(ARM_STATE_FEATURE_DIM + GRIPPER_STATE_FEATURE_DIM, self.hidden_dim) # 384
        ...
        self.action_projector = nn.Linear(ARM_STATE_FEATURE_DIM + GRIPPER_STATE_FEATURE_DIM, self.hidden_dim) # 384 
        ...
        self.NUM_RESAMPLER_QUERY = num_resampler_query # 9
        self.perceiver_resampler = PerceiverResampler(dim=self.RESAMPLER_hidden_dim, num_latents=self.NUM_RESAMPLER_QUERY, depth=3) # (b, T, n=9, D)
        ...
        self.attention_mask = nn.Parameter(generate_attention_mask_my(
                                    ...
                                    requires_grad=False)
        num_non_learnable_token_per_timestep = 1+1+1+self.NUM_RESAMPLER_QUERY*2+1*2
        ...
        self.NUM_MASK_TOKEN = int(input_image_size**2 / patch_size / patch_size)  # i.e. num_patch
    ...
    def forward(self, image_primary_1, image_primary_2, image_wrist, state, text_token, action):  
        ...
        # image_primary shape torch.Size([1, 10, 6, 180, 320])
        # image_wrist shape torch.Size([1, 10, 3, 180, 320])
        # state shape torch.Size([1, 10, 7])
        # text_token torch.Size([1, 10, 77])
        # attention_mask torch.Size([370, 370])

        device = image_primary_1.device
        S_AND_FUTURE = image_primary_1.shape[1]
        image_pred = None 
        ...
        action_embeddinig = self.action_projector(torch.cat((arm_action_feature, gripper_action_feature), dim=1))
        action_embeddinig = action_embeddinig.view(B, S, -1, self.hidden_dim)
        ...
        # image feature 
        if image_primary_1.type() != self.vision_encoder_type:
            image_primary_1 = image_primary_1.type(self.vision_encoder_type)
            image_primary_2 = image_primary_2.type(self.vision_encoder_type)
            image_wrist = image_wrist.type(self.vision_encoder_type)
        
        with torch.no_grad():
            image_primary_feature_1, _, _ = self.vision_encoder.forward_encoder(image_primary_1.flatten(0, 1), mask_ratio=0.0)
            image_primary_feature_2, _, _ = self.vision_encoder.forward_encoder(image_primary_2.flatten(0, 1), mask_ratio=0.0)
            image_wrist_feature, _, _ = self.vision_encoder.forward_encoder(image_wrist.flatten(0, 1), mask_ratio=0.0)
        
        if image_primary_feature_1.type() != self.perceiver_resampler_type:
            image_primary_feature_1 = image_primary_feature_1.type(self.perceiver_resampler_type)
            image_primary_feature_2 = image_primary_feature_2.type(self.perceiver_resampler_type)
            image_wrist_feature = image_wrist_feature.type(self.perceiver_resampler_type)

        image_primary_feature_1 = image_primary_feature_1.view(B, S_AND_FUTURE, image_primary_feature_1.shape[-2], image_primary_feature_1.shape[-1])      
        image_primary_feature_2 = image_primary_feature_2.view(B, S_AND_FUTURE, image_primary_feature_2.shape[-2], image_primary_feature_2.shape[-1])      

        image_wrist_feature = image_wrist_feature.view(B, S_AND_FUTURE, image_wrist_feature.shape[-2], image_wrist_feature.shape[-1])
        image_primary_1_cls_token = image_primary_feature_1[:, :, :1, :]
        image_primary_2_cls_token = image_primary_feature_2[:, :, :1, :]
        image_wrist_cls_token = image_wrist_feature[:, :, :1, :]
        image_primary_1_feature = image_primary_feature_1[:, :, 1:, :]
        image_primary_2_feature = image_primary_feature_2[:, :, 1:, :]
        ...
        # perceiver resampler
        image_primary_1_feature = self.perceiver_resampler(image_primary_1_feature.reshape(B*S, 196, self.RESAMPLER_hidden_dim).unsqueeze(1).unsqueeze(1))  # mae vit outputs 196 tokens
        image_primary_2_feature = self.perceiver_resampler(image_primary_2_feature.reshape(B*S, 196, self.RESAMPLER_hidden_dim).unsqueeze(1).unsqueeze(1))  # mae vit outputs 196 tokens
        ...
        image_primary_1_embedding = self.image_primary_projector(image_primary_1_feature.flatten(0, 2)).view(B, S, -1, self.hidden_dim)
        image_primary_2_embedding = self.image_primary_projector(image_primary_1_feature.flatten(0, 2)).view(B, S, -1, self.hidden_dim)
        image_wrist_embedding = self.image_wrist_projector(image_wrist_feature.flatten(0, 2)).view(B, S, -1, self.hidden_dim)
        
        image_embedding = torch.cat((image_primary_1_embedding, image_primary_2_embedding, image_wrist_embedding), dim=2)
        image_cls_token_primary_1_embedding = self.cls_token_primary_projector(image_primary_1_cls_token.flatten(0, 2)).view(B, S, -1, self.hidden_dim)
        image_cls_token_primary_2_embedding = self.cls_token_primary_projector(image_primary_2_cls_token.flatten(0, 2)).view(B, S, -1, self.hidden_dim)
        ...
        image_cls_token_embedding = torch.cat((image_cls_token_primary_1_embedding, image_cls_token_primary_2_embedding, image_cls_token_wrist_embedding), dim=2)
        
        # aggregate embeddings and add timestep position encoding
        embeddings = torch.cat((text_embedding, state_embedding, action_embeddinig, image_embedding, image_cls_token_embedding), dim=2) ## (1+1+self.NUM_RESAMPLER_QUERY*2+1*2)
        ...
        return arm_pred_action, gripper_pred_action, image_pred
