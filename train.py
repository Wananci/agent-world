...
from utils.train_utils import get_checkpoint, train_one_epoch_droid, get_ckpt_name
from utils.arguments_utils import get_parser
from utils.droid_data_utils import *
...
def main(args):  
    ...
    if args.finetune_type == "droid":
        dataset = get_mydroid_dataset(args, model.image_processor, clip, epoch=0, split='train')
    else:
        raise ValueError
    ...
    for epoch in range(resume_from_epoch, args.num_epochs):
        ...
        droid_loader = dataset.dataloader
        train_one_epoch_droid(
            args=args,
            model=ddp_model,
            epoch=epoch,
            optimizer=optimizer,
            lr_scheduler=lr_scheduler,
            droid_loader=droid_loader,
            device_id=device_id,
        )
